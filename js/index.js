...
const comparationColor = (color1, color2, sortKind) => {
  // TODO: допишите функцию сравнения двух элементов по цвету
  const priority = ['желтый', 'зеленый', 'розово-красный', 'светло-коричневый', 'фиолетовый']; // сравнение по алфавиту
  const priority1 = priority.indexOf(color1);
  const priority2 = priority.indexOf(color2);
  return sortKind === 'bubbleSort' ? priority1 > priority2 : priority1 <= priority2;
};
...
const sortAPI = {
  bubbleSort(arr, comparation) {
    ...
    for (let i = 0; i < n; i++) {
      for (let j = 0; j < n - i; j++) {
        if (comparation(arr[j].color, arr[j + 1].color, sortKind)) {
  ...
  quickSort(arr, comparation) {
    ...
    const partition = (arr, start, end) => {
      let pivotValue = arr[end].color;
      let pivotIndex = start;
      for (let j = start; j < end; j++) {
        if (comparation(arr[j].color, pivotValue, sortKind)) {
    ...
    const quickSortRecursive = (arr, start, end) => {
      if (start < end) {
        const indexElement = partition(arr, start, end);
        quickSortRecursive(arr, start, indexElement - 1);
        quickSortRecursive(arr, indexElement + 1, end);
      }
    }
...
sortActionButton.addEventListener('click', () => {
  ...
  // возвращаем значение вида сортировки
  setTimeout(() => {
    sortKindLabel.textContent = sortKind;
  }, 1000);
